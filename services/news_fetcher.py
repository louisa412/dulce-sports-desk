...
from config import RECENT_HOURS, BACKFILL_HOURS, CATEGORY_NEWS_COUNT
...
def score_arsenal(title, summary):
    ...
    score = 0
    if any(k in text for k in ["injury", "injured", "fitness", "return"]):
        score += 4
    if any(k in text for k in ["transfer", "sign", "bid", "deal"]):
        score += 3
    if any(k in text for k in ["arteta", "press conference", "lineup"]):
        score += 2
    if any(k in text for k in ["arsenal", "gunners", "saka", "odegaard"]):
        score += 1
    return score
...
def score_spain(title, summary):
    text = f"{title} {summary}".lower()
    score = 0
    if any(k in text for k in ["squad", "call-up", "selected"]):
        score += 4
    if any(k in text for k in ["coach", "manager", "tactics"]):
        score += 2
    return score
...
def score_f1(title, summary):
    text = f"{title} {summary}".lower()
    score = 0
    if any(k in text for k in ["ferrari", "leclerc"]):
        score += 2
    if any(k in text for k in ["qualifying", "race", "pole"]):
        score += 3
    return score
...
def pick_top_news(entries, scorer, top_n=CATEGORY_NEWS_COUNT):
    ...
    for e in entries:
        if e["title"] in seen or e["published_at"] is None or e["published_at"] < cutoff_48h:
            continue
        seen.add(e["title"])
