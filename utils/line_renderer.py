from config import SECTION_ACCENT_COLOR, OVERVIEW_ACCENT_COLOR
...
def build_news_item_component(item, index):
    return [
        ...
        },
        {
            ...
            "style": "link",
            "height": "sm",
            "action": {"type": "uri", "label": "閱讀原文", "uri": item["link"]},
        },
        {"type": "separator", "margin": "md"},
    ]


def build_overview_bubble(text):
    overview_text = text if isinstance(text, str) and text.strip() else "今日重點整理中，稍後補上。"
    return {
        "type": "bubble",
        "size": "giga",
        "header": {
            "type": "box",
            "layout": "vertical",
            "backgroundColor": OVERVIEW_ACCENT_COLOR,
            "paddingAll": "16px",
            "contents": [
                {"type": "text", "text": "今日精華總覽", "weight": "bold", "size": "xl", "color": "#FFFFFF"}
            ],
        },
        "body": {
            "type": "box",
...
def build_section_bubble(title, items, color):
    ...
    if body and body[-1]["type"] == "separator":
        body.pop()
